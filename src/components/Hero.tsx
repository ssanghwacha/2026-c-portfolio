    ...
    <>
      ...
      {status.icons?.map(({ src, alt }, index) => (
        <Image
          ...
          width={28}
          height={28}
          className={`size-6 rounded-[4px] object-contain lg:size-7 ${index === 0 ? 'ml-2.5' : ''}`}
        />
      ))}
    </>
  );
...
function NowStatusLine() {
  ...
  return (
    <p
      aria-live="polite"
      className="mx-auto mt-8 flex min-h-9 w-[calc(100vw-24px)] flex-col items-center justify-center gap-2 text-center text-[16px] leading-[1.2] tracking-[-0.01em] text-primary sm:mt-10 sm:w-full sm:min-h-9 sm:flex-row sm:gap-1.5 lg:w-auto lg:min-h-10 lg:gap-2 lg:text-[20px] dark:text-[#E6E6E6]"
    ...
    >
      ...
      <motion.span
        ...
        className="relative inline-flex min-h-8 max-w-full overflow-hidden rounded-full bg-primary px-3 py-0.5 text-white will-change-[width] sm:min-h-9 sm:py-1.5 lg:min-h-10 lg:px-3.5 lg:py-1 dark:bg-[#E6E6E6] dark:text-[#1E1E1E]"
      >
        ...
        <AnimatePresence mode="sync" initial={false}>
          <motion.span
            ...
            className="absolute inset-y-0 left-3 right-3 inline-flex items-center justify-center gap-1 whitespace-nowrap lg:left-3.5 lg:right-3.5 lg:gap-1.5"
          >
            <StatusContent status={status} />
          </motion.span>
        </AnimatePresence>
...
export default function Hero() {
  return (
    <section className="relative isolate flex min-h-[90svh] touch-pan-y items-center overflow-hidden bg-white px-4 py-16 text-[#1E1E1E] sm:min-h-[70svh] sm:px-10 sm:py-24 lg:min-h-[90vh] lg:px-[5.5vw] lg:py-24 dark:bg-[#1E1E1E] dark:text-[#E6E6E6]">
      <div className="relative z-10 mx-auto w-full max-w-[1540px] -translate-y-[4svh] text-center sm:translate-y-[4svh] lg:translate-y-[6svh]">
        <h1
          className="mx-auto max-w-[1450px] text-[clamp(26px,7vw,36px)] font-medium text-[#252525] sm:text-[clamp(30px,5vw,56px)] lg:text-[clamp(53px,5.15vw,78px)] lg:font-normal dark:text-[#E6E6E6]"
          style={{
        ...
        >
          <span className="sm:hidden">
            {[
              ...
              <>shaping brands, digital</>,
              <>products, and systems with</>,
              <>a refined eye for user experience.</>,
            ].map((line, index) => (
              <motion.span
                key={index}
                className="block"
